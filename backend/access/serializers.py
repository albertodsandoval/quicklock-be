...
class KeyGenerationSerializer(ModelSerializer):
	...
	def validate(self, data):
		if data['not_valid_after'] is not None and data['not_valid_after'] < data['not_valid_before']:
		...
		if has_access:
			raise serializers.ValidationError("User already has access to this lock. Update their key instead.")
	...
	def validate_administrator(self, administrator):
		try:
			...
			if not administrator.is_staff:
				raise ValidationError("Signed in user is not an administrator.")
